...
function assertSafeImportId(importId: string) {
  if (!/^[a-zA-Z0-9_-]{1,80}$/.test(importId)) {
    throw new Error("历史解析任务 ID 不合法。");
  }
}

function isZipSymlink(entry: IZipEntry) {
  const mode = (entry.attr >>> 16) & 0o170000;
  return mode === 0o120000;
}

function resolveInside(rootDir: string, entryName: string) {
...
async function extractSafeEmlEntries(bytes: Buffer, extractedDir: string) {
