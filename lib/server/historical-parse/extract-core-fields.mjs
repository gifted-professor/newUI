...
function buildSummary(fullBody) {
  const normalized = normalizeWhitespace(fullBody).replace(/\n/g, " ");
  return normalized.slice(0, 240);
}

function detectPlatform(subject, body) {
  const source = `${subject}\n${body}`;
  const matches = PLATFORM_PATTERNS.map((item) => ({
    platform: item.platform,
    score: item.patterns.reduce((count, pattern) => count + ((source.match(pattern) || []).length > 0 ? 1 : 0), 0),
  }))
...
function detectQuotedPrice(fullBody) {
...
function detectIntent(body, quotedPrice) {
...
function detectStage(subject, body, quotedPrice, intent) {
  const source = `${subject}\n${body}`;
...
function normalizeCreatorCandidate(value) {
...
function detectCreatorId(subject, from, to, fullBody, fromAddress, toAddress) {
  ...
  if (fromAddress && !isInternalAddress(fromAddress) && fromLocal) {
    return fromLocal;
  }

  if (fromAddress && isInternalAddress(fromAddress) && toAddress && !isInternalAddress(toAddress) && toLocal) {
    return toLocal;
  }
  ...
  if (normalizedHandles.length === 1) {
    return normalizedHandles[0];
  }

  const greeting = normalizeCreatorCandidate(extractGreetingCandidate(fullBody));
  if (greeting && fromAddress && isInternalAddress(fromAddress)) {
    return greeting;
  }
...
export function extractCoreFields(parsedBody) {
  ...
  return {
    ...
    platform,
    quotedPrice,
    latestReplyAt: parsedBody.date || null,
    stage,
    intent,
    summary,
    needsManualReview,
