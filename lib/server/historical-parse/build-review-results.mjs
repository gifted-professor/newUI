...
function chooseConfidenceLabel(confidence) {
  if (confidence >= 0.78) return "high";
  if (confidence >= 0.55) return "medium";
  return "low";
}

function pickLatestStatus(current, candidate, currentAt, candidateAt) {
...
function buildReviewReasons(group, creatorId) {
  const reasons = [];
...
function scoreCreatorGroup(group, creatorId) {
  let score = 0.28;
...
export function buildCreatorProfiles(conversationGroups) {
  ...
  return [...profiles.values()]
    .map((profile) => {
      const averageConfidence = profile.confidenceScores.length
        ? profile.confidenceScores.reduce((sum, item) => sum + item, 0) / profile.confidenceScores.length
        : 0.1;
